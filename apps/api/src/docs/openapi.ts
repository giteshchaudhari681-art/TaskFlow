...
export const openApiSpec = {
  ...
  components: {
    ...
    schemas: {
      ...
      AuditSourceEnum: {
        type: 'string',
        enum: ['USER', 'SYSTEM', 'AI', 'AI_ASSISTED'],
      },
      AuditActionEnum: {
        type: 'string',
        enum: [
      ...
      AuditEvent: {
        ...
        properties: {
          ...
          actorType: { $ref: '#/components/schemas/ActorTypeEnum' },
          action: { $ref: '#/components/schemas/AuditActionEnum' },
          resourceType: { type: 'string', example: 'Task' },
          resourceId: { type: 'string', nullable: true },
          requestId: { type: 'string', nullable: true },
          source: { $ref: '#/components/schemas/AuditSourceEnum' },
          ...
          project: {
            ...
            properties: {
              id: { type: 'string', format: 'uuid' },
              name: { type: 'string', example: 'Alpha Project' },
              key: { type: 'string', example: 'ALPHA' },
            },
          },
        },
      },
      AuditEventsResponse: {
        ...
        properties: {
          ...
          meta: {
            type: 'object',
            required: ['page', 'limit', 'total', 'totalPages', 'timestamp'],
            properties: {
              page: { type: 'integer', example: 1 },
  ...
  paths: {
    ...
    '/api/v1/organizations/{organizationId}/audit-events': {
      get: {
        tags: ['Audit', 'Organizations'],
        summary: 'Query workspace audit events',
        description:
          'Query append-only audit trail and security events with bounded filters and pagination. Requires Organization OWNER/ADMIN or Project ADMIN permissions.',
        security: [{ bearerAuth: [] }],
        parameters: [
          { $ref: '#/components/parameters/organizationIdParam' },
          {
            ...
            in: 'query',
            required: false,
            schema: { $ref: '#/components/schemas/AuditActionEnum' },
          ...
          },
          {
            ...
            required: false,
            schema: { type: 'string', format: 'uuid' },
            description: 'Filter by UUID of the actor user',
          },
          {
            name: 'projectId',
            in: 'query',
            required: false,
            schema: { type: 'string', format: 'uuid' },
            description: 'Filter by project UUID within the workspace',
          },
          {
            name: 'resourceType',
            in: 'query',
            required: false,
            schema: { type: 'string' },
            description: 'Filter by resource type (e.g. Task, Project, User, Session)',
          },
          {
            name: 'from',
            in: 'query',
            ...
            description: 'Start of ISO 8601 creation timestamp range',
          },
          {
            name: 'to',
            in: 'query',
            required: false,
            schema: { type: 'string', format: 'date-time' },
            description: 'End of ISO 8601 creation timestamp range',
          },
          {
            name: 'page',
            in: 'query',
          ...
          },
          {
            ...
            description: 'Maximum audit records per page (default 25, max 100)',
          },
        ],
        responses: {
          200: {
            description: 'Filtered list of audit events with pagination metadata',
            content: {
              'application/json': {
