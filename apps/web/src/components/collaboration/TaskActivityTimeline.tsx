    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <History className="w-4 h-4 text-taskflow-accent" />
      ...
      {!loading && !error && activities.length === 0 && (
        <div className="text-center py-6 px-4 bg-slate-950/30 border border-slate-800/60 rounded-md">
          ...
          {activities.map(activity => {
            ...
            return (
              <div key={activity.id} className="relative group">
                {/* Dot */}
                <div className="absolute -left-[19px] top-1 w-2.5 h-2.5 rounded-full bg-slate-900 border-2 border-taskflow-accent group-hover:border-taskflow-accent group-hover:scale-110 transition-all" />
