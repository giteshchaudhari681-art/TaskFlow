...
function getMonthHeaders(start: Date, end: Date): Array<{ label: string; left: number; width: number }> {
...
export const TimelineView: React.FC<TimelineViewProps> = ({ organizationId, projectId }) => {
  ...
  const todayOffset = ((new Date().getTime() - displayStart.getTime()) / (displayEnd.getTime() - displayStart.getTime())) * 100;
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-taskflow-muted mt-0.5">
            {formatShort(timeline.rangeStart)} → {formatShort(timeline.rangeEnd)} · {timeline.milestones.length} milestone{timeline.milestones.length !== 1 ? 's' : ''}
          </p>
        </div>
        <button onClick={fetchTimeline} className="p-2 rounded-xl text-taskflow-muted hover:text-white hover:bg-taskflow-surface border border-taskflow-border transition-colors" aria-label="Refresh timeline" title="Refresh">
      ...
      <div className="glass-panel rounded-2xl border border-taskflow-border bg-taskflow-surface overflow-hidden">
        ...
        <div className="divide-y divide-taskflow-border/20">
          {milestonesWithDates.map((ms, idx) => {
            ...
            return (
              <div key={ms.id} className={`flex items-center min-h-[52px] ${idx % 2 === 0 ? 'bg-taskflow-surface' : 'bg-taskflow-bg/30'}`}>
                {/* Left label */}
                <div className="w-48 flex-shrink-0 px-4 py-2 border-r border-taskflow-border/30">
                  ...
                  <div className="flex items-center gap-1.5 mt-0.5">
                    <span className={`text-[10px] font-semibold ${textClass}`}>{ms.health.replace('_', ' ')}</span>
        ...
        {milestonesNoDates.length > 0 && (
          <div className="border-t border-taskflow-border/40 p-4">
            <p className="text-[10px] uppercase font-bold text-taskflow-muted mb-2">No Date Set ({milestonesNoDates.length})</p>
            <div className="flex flex-wrap gap-2">
              {milestonesNoDates.map(ms => (
                <span key={ms.id} className="px-2.5 py-1 rounded-lg bg-taskflow-bg text-xs text-taskflow-muted border border-taskflow-border">
                  {ms.title}
                </span>
              ))}
            </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        {timeline.milestones.slice(0, 3).map(ms => (
          <div key={ms.id} className="glass-panel rounded-xl border border-taskflow-border p-4 bg-taskflow-surface/40 space-y-2">
            <div className="flex items-start justify-between gap-2">
              <p className="text-xs font-semibold text-white truncate">{ms.title}</p>
              <span className={`text-[10px] font-bold flex-shrink-0 ${HEALTH_TEXT[ms.health as string] ?? 'text-taskflow-muted'}`}>
                {ms.health.replace('_', ' ')}
              </span>
            </div>
            <div className="space-y-1">
            ...
            <div className="flex items-center gap-1.5 text-[10px] text-taskflow-muted">
              ...
              <span>{ms.completedTaskCount}/{ms.taskCount} tasks</span>
            </div>
          </div>
        ))}
      </div>
