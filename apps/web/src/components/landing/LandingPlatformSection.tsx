      ...
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-10 relative z-10">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-12">
          {specs.map(({ icon: Icon, title, description, color, bg, border, glow }) => (
            <div
              key={title}
              className={`group p-6 rounded-2xl bg-white/[0.02] border ${border} hover:shadow-lg ${glow} transition-all duration-300 hover:-translate-y-0.5 space-y-4`}
            >
              <div
        ...
        {/* Tech stack strip */}
        <div className="p-6 sm:p-8 rounded-2xl bg-slate-950/60 border border-white/[0.07]">
          ...
          <div className="flex flex-wrap gap-3">
            {stackItems.map(({ label, cat }) => (
              <div
                key={label}
                className="flex items-center gap-2 px-3.5 py-2 rounded-xl bg-white/[0.04] border border-white/[0.07] hover:border-white/[0.12] transition-colors"
              >
