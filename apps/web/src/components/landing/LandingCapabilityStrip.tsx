...
export const LandingCapabilityStrip: React.FC = () => {
  return (
    <div className="border-y border-white/[0.06] bg-gradient-to-b from-slate-900/30 to-transparent py-12">
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-10">
        <div className="flex flex-wrap items-center justify-center gap-3">
          {caps.map(({ icon: Icon, label, color, bg, border }) => (
            <div
              key={label}
              className={`inline-flex items-center gap-2.5 px-4 py-2.5 rounded-xl ${bg} border ${border} group hover:scale-105 transition-transform duration-200`}
            >
